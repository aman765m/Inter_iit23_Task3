from swarm_pose_est import camera_pose
...
class quadControl():
    ...
    def __init__(self,ip):
        ...
        self.prev_position_array = np.zeros(3)
        self.vel_array = np.zeros(3)
        # self.pos = camera_pose()
        # temp = self.pos.getPose()
        # self.prev_position_array = temp[self.droneID-1,:]
    ...
    def posControl(self,curr_pos_arr,x,y,z,xvel,yvel,zvel,disarm):
        ...
        self.getCurrentPose(curr_pos_arr) # Get the current pose of the drone
    ...
    def getCurrentPose(self,curr_pos_arr):

        self.position_array[:] = curr_pos_arr
